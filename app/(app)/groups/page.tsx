...
import Sidebar from "@/components/MessagesSidebar";
...
export default function GroupsPage() {
  // selectedChat ab null se start hoga
  const [selectedChat, setSelectedChat] = useState<any>(null);
  const {appUser} = useAppUser()

  const fetchChats = async () => {
    const response = await axios.get(`/api/chats?isGroup=${true}?userId=${appUser?._id}`,); 
  }
  ...
  return (
    <main className="flex h-[calc(100vh-110px)] overflow-hidden bg-background border border-border rounded-3xl mb-4 shadow-xl ">
      ...
      `}>
        ...
        <ChatView activeChat={selectedChat} />
      </section>
    </main>
  );
}